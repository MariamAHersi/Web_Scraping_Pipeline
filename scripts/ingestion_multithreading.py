...
# Use ThreadPoolExecutor to speed up scraping using multithreading
# Instead of fetching book details one at a time, a pool of worker threads is created
# Each thread will execute the get_book_information() function concurrently on different book URLs
# The parameter max_workers=10 means that up to 10 requests can be sent at the same time, reducing waiting time
